...
int main() {
    ...
    int filter_size = 3; // Fixed filter size
    int input_depth = 1;
    ...
    for (int input_size : image_sizes) {
        ...
        conv1->output_width = (input_size - conv1->filter_width + 2 * conv1->pad) / conv1->stride + 1;
        conv1->output_height = (input_size - conv1->filter_width + 2 * conv1->pad) / conv1->stride + 1;
        ...
        for (int i = 0; i < batch_size; ++i) {
            ...
            test_data[i]->weights = (double *)malloc(input_size * input_size * input_depth * sizeof(double));
            test_data[i]->width = input_size;
            test_data[i]->height = input_size;
            test_data[i]->depth = input_depth;
            for (int j = 0; j < input_size * input_size * input_depth; ++j) {
